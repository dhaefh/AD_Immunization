...
# Summary: Generate LOESS predictions for iAD-Ext and iAD-Lim gene expression based on amyloid density in the plaque niche, for genes in iAD cluster 4
#
#-------------------------------------------------------------------------------
...
# Generate predictions for iAD-Lim
cur_data <- data[data$condition == "lim", c(clust_genes, "amyloid")]
...
write.csv(merged, paste0(output_folder, "lim_predictions_clust4.csv"), row.names = FALSE)

# Generate predictions for iAD-Ext
cur_data <- data[data$condition == "ext", c(clust_genes, "amyloid")]
predictions <- lapply(clust_genes, run_loess, data = cur_data)
merged <- as.data.frame(predictions)
merged$amyloid <- seq(min(cur_data$amyloid), max(cur_data$amyloid), 1)
merged <- dplyr::relocate(merged, amyloid) 
write.csv(merged, paste0(output_folder, "ext_predictions_clust4.csv"), row.names = FALSE)
