...
s@meta.data <- s@meta.data %>% mutate(region = case_when(substr(s$sample_merged, nchar(s$sample_merged), nchar(s$sample_merged)) == 1 ~ "FCX",
                                                         substr(s$sample_merged, nchar(s$sample_merged), nchar(s$sample_merged)) == 3 ~ "TCX",
                                                         substr(s$sample_merged, nchar(s$sample_merged), nchar(s$sample_merged)) == 4 ~ "PCX",
                                                         substr(s$sample_merged, nchar(s$sample_merged), nchar(s$sample_merged)) == 9 ~ "HIPP"))

# Define broad donor variable
s$donor <- NA
s$donor[grep("^A14.193", s$sample_merged)] <- "A14.193"
s$donor[grep("^A11.170", s$sample_merged)] <- "A11.170"
s$donor[grep("^NMA22.300.A", s$sample_merged)] <- "NMA22.A"
s$donor[grep("^NMA22.205.B", s$sample_merged)] <- "NMA22.B"
