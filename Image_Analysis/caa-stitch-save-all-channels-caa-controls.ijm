...
- [ ]  AN1792
    ...
    - [ ]  Send out NanoString antibody for CosMx
    - [ ]  Follow up about RNAscope
    - [ ]  Backup all new images to fsmres
    - [ ]  
    - [x]  Finish staining
        - [x]  Secondary incubation
            - [x]  Update Benchling
        - [x]  DAPI
        - [x]  TruBlack
        - [x]  Mounting
    - [ ]  HD stains
        ...
        - [ ]  Quantify Iba1 coverage in HD samples
        - [ ]  Quantify vascular density in HD samples
        - [ ]  **Quantify amyloid (vascular and cortical separately) coverage in HD samples**
        - [ ]  **Quantify amount of Iba1 overlapping amyloid**
        - [x]  Clean manually thresholded amyloid
            - [x]  Download to desktop
            - [x]  Redo thresholding to include more ??
            - [x]  Determine cleaning pipeline
        - [ ]  Isolate vascular amyloid using ULEX
        - [ ]  Isolate DAPI channel for missing half
        - [ ]  Isolate cortical amyloid
        - [ ]  Finish all tissue traces
            - [ ]  Have Lynn check
        - [ ]  Start from scratch with Iba1 images
            - [x]  Try stitch without bleach correction
            - [ ]  Try bleach correct already stitched images
            - [ ]  Background subtract
            - [ ]  Auto-threshold and filter
    - [ ]  CAA controls
        - [x]  Downsize amyloid multipage tifs on sequencing computer
        - [x]  Overlay cleaned binary iba1 selections on bleached to show Lynn
        - [ ]  Make iba1 multipage tif
            - [ ]  Write script// Alex Edwards, 7-10-2024
...
//					Downsamples each tile to ensure stitched image does not surpass 2 gigapixel image limit
...
function bleaching(channelDir, mainDir, channelName) {
	...
	//File.openSequence(channelDir); // supposed to be able to open folder as a stack, but doesn't work in headless mode
	...
	for (panel = 0; panel < numSlices; panel++) {
		saving = getTitle();
		saveAs("tif", bleachedImages + saving);
		close();
	}
...
function establishesTilePositions(tilesDir, outputDir, sampleName, channel) {
	...
	savedTiles = getFileList(outputFolder);
	for (xPositions = 1; xPositions < xValues.length + 1; xPositions++) {
		for (yPositions = 1; yPositions < yValues.length + 1; yPositions++) {
			print("(" + xPositions + ", " + yPositions + ")");
			if (File.exists(outputFolder + "(" + xPositions + ", " + yPositions + ").tif") != 1) {
				newImage("new", "8-bit black", 2048, 1536, 1);
				saveAs("tif", outputFolder + "(" + xPositions + ", " + yPositions + ")");
				print("Created tile for (" + xPositions + ", " + yPositions + ")");
			}
			else {
				print("(" + xPositions + ", " + yPositions + ").tif already exists");
			}
		}
	}
...
function downsampleTiles(coordinateTilesFolder, outputDir) {
	outputFolder = outputDir + sampleName + "-ch" + channel + "-downsampled-renamed-w-coords-final/";
	File.makeDirectory(outputFolder);
	
	imageList = getFileList(coordinateTilesFolder);
	print(imageList.length);
	
	for (tiles = 0; tiles < imageList.length; tiles++) {
		currentImage = imageList[tiles];
		open(coordinateTilesFolder + currentImage);
		title = getTitle();
		coordinate = title.substring(0, title.length - 4);
		print("Current image is " + title);
		
		run("Scale...", "x=- y=- width=1600 height=1200 interpolation=Bilinear average create title=[" + coordinate + "-downsampled.tif]");
		saveAs("tif", outputFolder + coordinate + "-downsampled");
		print("Saved downsampled tile " + coordinate + "(Image #" + tiles + ")");
	}
}
...
function bleachingStitchingRollingBall(channel) {
	...
	if (channel == 3) { // for iba1 images, perform bleach correction
...
sortedFolder = basePath + "/sorted-072424-FINAL/";
File.makeDirectory(sortedFolder);

// make sample specific sorted folder
sampleName = args.substring(secondLastForwardSlash + 1, args.length - 22); // removes the time and date info from the sample name
